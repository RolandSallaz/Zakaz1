...
export default function GameSelectionForm({ onSubmitForm, isEditing }: props) {
  ...
  const [gamesInSelection, setGamesInSelection] = useState<
    IGameSelectionInitState[]
  >([]);
  ...
  useEffect(() => {
    if (isEditing) {
      const gameSelection = gameSelections.find(
        (item) => item.id == Number(id)
      );
      if (gameSelection) {
        mutateValue({ valueName: "name", value: gameSelection.name });
        setGamesInSelection(
          gameSelection.games?.map((item, index) => ({
            index,
            gameId: item.digiId,
          }))
        );
